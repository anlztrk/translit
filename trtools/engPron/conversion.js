function cyrlat() {
	document.transcription.text2.value = document.transcription.text1.value
		...
		.replace(/[oœ]ː?([bʤʧdfkclɫmpɾʃtvβz](?![bʤʧdfɡɟʒkclɫmnŋɲpɾsʃtvβz]))/g, 'OA$1')
		.replace(/[oœ]ː?ʤ/g, 'OADGE')
		.replace(/[oœ]ː?n/g, 'OAN')
		.replace(/[oœ]ː?s(?![bʤʧdfɡɟʒkclɫmnŋɲpɾsʃtvβz])/g, 'OSS')
		.replace(/[oœ]ː?/g, 'OH')
		.replace(/(?<![bʤʧdfɡhɟʒkclɫmnŋɲpɾsʃtvβjz])ɑɾ(?![bʤʧdfɡhɟʒkclɫmnŋɲpɾsʃtvβjz])/g, 'ARE')
		.replace(/ɑɾ/g, 'AR')

		.replace(/ɑ([fnts])(?![bʤʧdfɡɟʒkclɫmnŋɲpɾstʃvβz])/g, 'U$1$1')
		.replace(/ɑ(?=[bʤdfɡɟʒmnŋɲpstvβz])/g, 'U')
		.replace(/ɑ[ck]/g, 'UCK')
		.replace(/ɑʤ/g, 'UDGE')
		.replace(/ɑ(?=[bʤʧdfɡɟʒkclɫmnŋɲpɾstʃvβz])/g, 'AH')
		.replace(/(uː|u|yː|y)/g, 'OO')
		.replace(/ɑ/g, 'A')
		.replace(/ɯ/g, 'Ə')
		.replace(/(?<=[AEIOUYaeuioy])ʤ(?=E)/g, 'G')
		.replace(/(?<=[AEIOUYaeuioy])ʤ/g, 'DGE')
		.replace(/ʤ/g, 'J')
		.replace(/(?<=[AEIOUYaeuioy])ʧ(?=E)/g, 'CH')
		.replace(/(?<=[AEIOUYaeuioy])ʧ/g, 'TCH')
		.replace(/ʧ/g, 'CH')
		.replace(/[ɡɟ](?=[EIei])/g, 'GH')
		.replace(/[ɡɟ]/g, 'G')
		.replace(/ʒ/g, 'ZH')
		.replace(/[lɫ]/g, 'L')
		.replace(/(?:ŋk|ɲc)/g, 'NK')
