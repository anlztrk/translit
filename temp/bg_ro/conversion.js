...
car = car.replace(/Д(Ж|ж)/g, "Ӂ");
car = car.replace(/дж/g, "ӂ");
car = car.replace(/Й(О|о)/g, "Ё");
car = car.replace(/йо/g, "ё");

car = car.replace(/(\p{Uppercase})(\p{Uppercase}) Щ/ug, "$1$2 ШТ");
car = car.replace(/Щ (\p{Uppercase})(\p{Uppercase})/ug, "ШТ $1$2");
car = car.replace(/Щ(\p{Uppercase})/ug, "ШТ$1");
car = car.replace(/Щ/g, "Шт");
car = car.replace(/щ/g, "шт");
...
car = car.replace(/(А|Е|Ё|О|У|Ъ|Ю|Я)Я/g, "$1ИА");
car = car.replace(/(А|Е|Ё|О|У|Ъ|Ю|Я|а|е|ё|о|у|ъ|ю|я)я/g, "$1иа");
...
car = car.replace(/(Ь|ь)(А|а|Б|б|В|в|Г|г|Д|д|Е|е|Ё|ё|Ж|ж|Ӂ|ӂ|З|з|И|и|Й|й|К|к|Л|л|М|м|Н|н|О|о|П|п|Р|р|С|с|Т|т|У|у|Ф|ф|Х|х|Ц|ц|Ч|ч|Ш|ш|Щ|щ|Ъ|ъ|Ь|ь|Ю|ю|Я|я)/g, "$2");
...
car = car.replace(/ш/g, "ș");
car = car.replace(/Ъ/g, "Î");
car = car.replace(/ъ/g, "î");

car = car.replace(/(A|Â|B|C|D|E|F|G|H|I|Î|J|K|L|M|N|O|P|R|S|Ș|T|Ț|U|V|X|Z)Î(A|Â|B|C|D|E|F|G|H|I|Î|J|K|L|M|N|O|P|R|S|Ș|T|Ț|U|V|X|Z)/g, "$1Â$2");
car = car.replace(/(A|Â|B|C|D|E|F|G|H|I|Î|J|K|L|M|N|O|P|R|S|Ș|T|Ț|U|V|X|Z|a|â|b|c|d|e|f|g|h|i|î|j|k|l|m|n|o|p|r|s|ș|t|ț|u|v|x|z)î(a|â|b|c|d|e|f|g|h|i|î|j|k|l|m|n|o|p|r|s|ș|t|ț|u|v|x|z)/g, "$1â$2");
