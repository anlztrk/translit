function cyrlat() {
	document.transcription.text2.value = document.transcription.text1.value
		...
		.replace(/([AEIOUY])NY/g, "$1NNY")
		.replace(/([AEIOUYaeiouy])ny/g, "$1nny")
		
		.replace(/([AEIOUY])LY/g, "$1LLY")
		.replace(/([AEIOUYaeiouy])ly/g, "$1lly")

		.replace(/s([bdgĝlmnrvż])/g, "ż$1")
		.replace(/S([BDGĜLMNRVŻbdglmnrvż])/g, "Ż$1")
        ...
        .replace(/Ż/g, "Z")
		.replace(/h/g, "")
		.replace(/H(\p{Ll})/ug, (_, c) => c.toUpperCase())
		.replace(/H/g, "");
}
