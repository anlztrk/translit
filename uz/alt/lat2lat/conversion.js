...
car = car.replace(/(G|H|K|Y)O/g, "$1Ö");
car = car.replace(/(G|H|K|Y)U/g, "$1Ü");
car = car.replace(/(Ğ|X|Q|ğ|x|q)ä/g, "$1a");  
car = car.replace(/(G|H|K|Y|g|h|k|y)ı/g, "$1i");  
car = car.replace(/(G|H|K|Y|g|h|k|y)o/g, "$1ö");  
car = car.replace(/(G|H|K|Y|g|h|k|y)u/g, "$1ü");  

car = car.replace(/Ä(Ğ|X|Q|ğ|x|q)/g, "A$1");
car = car.replace(/I(G|H|K|Y|g|h|k|y)/g, "İ$1");
car = car.replace(/O(G|H|K|Y|g|h|k|y)/g, "Ö$1");
car = car.replace(/U(G|H|K|Y|g|h|k|y)/g, "Ü$1");
car = car.replace(/ä(ğ|x|q)/g, "a$1");
car = car.replace(/ı(g|h|k|y)/g, "i$1");
car = car.replace(/o(g|h|k|y)/g, "ö$1");
car = car.replace(/u(g|h|k|y)/g, "ü$1");

car = car.replace(/(G|H|K|Y|g|h|k|y)a/g, "$1ä");
