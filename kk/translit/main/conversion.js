function cyrlat() {
	...
	document.transcription.text2.value = document.transcription.text1.value
		...
		// VУ → Ў
		.replace(/([АОУҰЫӘЕИӨҮІ])У/g, "$1Ў")
		.replace(/([АОУҰЫӘЕИӨҮІаоуұыәеиөүі])у/g, "$1ў")

		// #УC → Ұ̂
		.replace(/(^|[^А-ЯӘӨҮҰҚҢҒҺІ])У([БВГҒДЖЗЙКҚЛМНҢПРСТФХШбвгғджзйкқлмнңпрстфхш])/g, "$1Ұ\u0302$2")
		.replace(/(^|[^А-ЯӘӨҮҰҚҢҒҺІа-яәөүұқңғһі])у([бвгғджзйкқлмнңпрстфхш])/g, "$1ұ\u0302$2")

		// #УV → Ў
		.replace(/(^|[^А-ЯӘӨҮҰҚҢҒҺІ])У([АОҰЫӘЕӨҮІИаоұыәеөүіи])/g, "$1Ў$2")
		.replace(/(^|[^А-ЯӘӨҮҰҚҢҒҺІа-яәөүұқңғһі])у([аоұыәеөүіи])/g, "$1ў$2")

		// fallback: isolated У
		.replace(/(^|[^А-ЯӘӨҮҰҚҢҒҺІа-яәөүұқңғһі])У($|[^А-ЯӘӨҮҰҚҢҒҺІа-яәөүұқңғһі])/g, "$1Ұ\u0302$2")
		.replace(/(^|[^А-ЯӘӨҮҰҚҢҒҺІа-яәөүұқңғһі])у($|[^А-ЯӘӨҮҰҚҢҒҺІа-яәөүұқңғһі])/g, "$1ұ\u0302$2")
