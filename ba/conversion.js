...
function cyrlat() {
	...
	car = car.replace(/([АЕЁИОӨУҮЫЮЯЭӘ])Ц/g, "$1ТС");
	car = car.replace(/([аеёиоөуүыюяэә])Ц/g, "$1Тс");
	car = car.replace(/([аеёиоөуүыюяэә])ц/g, "$1тс");
	car = car.replace(/Ц/g, "С");
	car = car.replace(/ц/g, "с");

	car = car.replace(/([АЕЁИОӨУҮЫЮЯЭӘ])Щ([АЕЁИОӨУҮЫЮЯЭӘ])/g, "$1ШШ$2");
	car = car.replace(/([АЕЁИОӨУҮЫЮЯЭӘ])щ([аеёиоөуүыюяэә])/g, "$1Шш$2");
	car = car.replace(/([аеёиоөуүыюяэә])щ([аеёиоөуүыюяэә])/g, "$1шш$2");
	car = car.replace(/щ/g, "ш");
	car = car.replace(/Щ/g, "Ш");

	car = car.replace(/([БВГҒДЖЗҘКҠЛМНҢПРСҪТФХҺЧШ])Я/g, "$1ЬА");
	car = car.replace(/([бвгғджзҙкҡлмнңпрсҫтфхһчш])Я/g, "$1ьА");
	car = car.replace(/([БВГҒДЖЗҘКҠЛМНҢПРСҪТФХҺЧШбвгғджзҙкҡлмнңпрсҫтфхһчш])я/g, "$1ьа");
	car = car.replace(/([БВГҒДЖЗҘКҠЛМНҢПРСҪТФХҺЧШбвгғджзҙкҡлмнңпрсҫтфхһчш])Ю/g, "$1Ү");
	car = car.replace(/([БВГҒДЖЗҘКҠЛМНҢПРСҪТФХҺЧШбвгғджзҙкҡлмнңпрсҫтфхһчш])ю/g, "$1ү");
	car = car.replace(/([БВГҒДЖЗҘЙКҠЛМНҢПРСҪТФХҺЧШбвгғджзҙйкҡлмнңпрсҫтфхһчш])Е/g, "$1Э");
	car = car.replace(/([БВГҒДЖЗҘЙКҠЛМНҢПРСҪТФХҺЧШбвгғджзҙйкҡлмнңпрсҫтфхһчш])е/g, "$1э");
	car = car.replace(/([БВГҒДЖЗҘКҠЛМНҢПРСҪТФХҺЧШбвгғджзҙкҡлмнңпрсҫтфхһчш])Ё/g, "$1Ө");
	car = car.replace(/([БВГҒДЖЗҘКҠЛМНҢПРСҪТФХҺЧШбвгғджзҙкҡлмнңпрсҫтфхһчш])ё/g, "$1ө");
	...
	car = car.replace(/([АЫОУ])У/g, "$1В");
	car = car.replace(/([АЫОУаыоу])у/g, "$1в");
	car = car.replace(/([ИЭӘӨҮ])Ү/g, "$1В");
	car = car.replace(/([ИЭӘӨҮиэәөү])ү/g, "$1в");
